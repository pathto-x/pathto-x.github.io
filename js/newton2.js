...
const processSVG = function (xmlDocumentElem) {
  ...
  const eachSvgLeftNav = document.querySelectorAll(".link_to_svg");
  const eachLinkResizeNav = document.querySelectorAll(".link_resize");


  const linkToSVGLeftNavClick = function (eventObject) {
...
const linkResizeClick = function (e) {
  var width = parseInt(e.target.dataset["width"], 10);
  window.state = width;
  console.log(width);
  helloworld(width)
};
eachSvgLeftNav.forEach(a => a.addEventListener('click', linkToSVGLeftNavClick))
eachLinkResizeNav.forEach(a => a.addEventListener('click', linkResizeClick))
